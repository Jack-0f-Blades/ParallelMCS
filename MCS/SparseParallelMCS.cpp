...
SparseParallelMCS::SparseParallelMCS(std::vector<std::vector<int>> const &adjacencyArray,
                                     ...
                                     int minTaskSize)
    : SparseMCS(adjacencyArray),
      m_taskQueue(queueCapacity == 0 ? std::max<size_t>(200, adjacencyArray.size()) : queueCapacity),
      m_queueCapacity(queueCapacity == 0 ? std::max<size_t>(200, adjacencyArray.size()) : queueCapacity),
...
{
    SetName("sparse-mcs-parallel");
    SetParallelMode(true);
    SetTaskQueue(&m_taskQueue);
    this->m_maxSplitDepth = m_maxSplitDepth;
    LOG("SparseParallelMCS created: queueCapacity=" + std::to_string(m_queueCapacity) +
...
SparseParallelMCS::~SparseParallelMCS()
{
    m_taskQueue.Stop();
}

long SparseParallelMCS::Run(std::list<std::list<int>> &cliques)
{
    LOG("=== Starting Sparse Parallel MCS ===");
    ...
    // Генерация начальных задач (только если целевой размер очереди не достигнут)
    ...
    // Основной поток обрабатывает оставшиеся задачи (если есть)
    ...
    while (!currentP.empty() || m_taskQueue.pending_tasks() > 0) {
        ...
        // Если очередь опустела, добавляем новые задачи из оставшихся начальных вершин
        ...
        if (taskOpt.has_value()) {
            ...
            // Извлекаем задачу и обрабатываем её
    ...
    LOG("=== Finished Sparse Parallel MCS, max clique size=" + std::to_string(m_uMaximumCliqueSize.load()) +
...
void SparseParallelMCS::RunFromTask(const Task& t,
...
void SparseParallelMCS::RunRecursiveParallel(std::vector<int> &P,
                                             std::vector<int> &vVertexOrder,
                                             std::list<std::list<int>> &cliques,
                                             std::vector<int> &vColors,
                                             std::vector<int> &R,
                                             ...
                                             long &nodeCount)
{
    // Локальная копия стратегии раскраски для потока
    SparseCliqueColoringStrategy localColoring(m_AdjacencyArray);
    ...
    while (!P.empty()) {
        ...
        // Периодический лог прогресса (для отладки)
        ...
        if (!newOrder.empty()) {
            std::vector<int> newP(newOrder.size());
            std::vector<int> newColors(newOrder.size());
            // Используем Recolor (как в MCS) с локальной стратегией
            localColoring.Recolor(m_AdjacencyArray, newOrder, newP, newColors,
            ...
            // Решение, создавать ли новую задачу
...
bool SparseParallelMCS::checkTimeout()
{
